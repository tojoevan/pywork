    ...
    async def list_comments(self, request, **kwargs) -> Any:
        ...
        if current_user_id:
            rows = await self.engine.fetchall(
                """SELECT c.*, COALESCE(u.nickname, u.display_name, u.username) as author_name, u.avatar as author_avatar
                   FROM comments c
        ...
        if top_ids:
            ...
            if current_user_id:
                all_replies = await self.engine.fetchall(
                    f"""SELECT c.*, COALESCE(u.nickname, u.display_name, u.username) as author_name, u.avatar as author_avatar
                       FROM comments c
                       LEFT JOIN users u ON c.author_id = u.id

...
### 1.4 LLM API Key 明文存储 ⏭ 待处理

**文件**: `plugins/llm_config/plugin.py:30-44`

`llm_configs` 表中 `api_key` 以明文存储。数据库泄露将导致所有 LLM API Key 泄露。

**修复建议**: 使用应用层加密（如 `cryptography.fernet`）加密存储，读取时解密。

### 1.5 MCP Token 明文存储 ⏭ 待处理

**文件**: `plugins/auth/plugin.py:1060-1067`

MCP Token 以明文存储在 `mcp_tokens` 表中。建议至少存储哈希值，仅在创建时返回完整 Token 一次。
...
### 2.1 大量裸 `except` 捕获 ⏭ 待处理

全项目存在 **20+ 处** 裸 `except:` 或 `except Exception` 吞掉所有异常且不记录日志：

| 文件 | 行号 | 上下文 |
|------|------|--------|
| `plugins/auth/plugin.py` | 69-71 | ALTER TABLE 容错 |
| `plugins/auth/plugin.py` | 97-104 | MCP tokens 表迁移 |
| `plugins/auth/plugin.py` | 237 | 密码解析 |
| `plugins/auth/plugin.py` | 438-439 | GitHub ID 查询 |
| `plugins/auth/plugin.py` | 452-453 | GitHub ID 更新 |
| `plugins/auth/plugin.py` | 478-479 | 用户创建 |
| `plugins/blog/plugin.py` | 266-267 | JSON 解析 tags |
| `plugins/blog/plugin.py` | 492-493 | JSON 解析 tags |
| `plugins/blog/plugin.py` | 537-538 | JSON 解析 tags |
| `plugins/microblog/plugin.py` | 273-278 | 请求体解析 |
| `app/config.py` | 215-216 | site_config 读取 |
| `app/config.py` | 286-287 | 配置迁移写入 |
| `app/log.py` | 101-102 | event loop 获取 |
| `app/log.py` | 114-115 | 日志写入 |

**影响**: 隐藏真实错误，使调试极为困难。至少应添加 `except Exception as e` 并记录日志。
...
### 2.6 `engine.execute()` 自动 commit ⏭ 待处理

**文件**: `app/storage/sqlite_engine.py:725-728`

```python
async def execute(self, sql: str, params: tuple = ()) -> None:
    await self._db.execute(sql, params)
    await self._db.commit()  # 每次都 commit
```

批量操作（如 `batch_set`、`_handle_hot_tags` 清空+重写）无法保证原子性，且频繁 commit 影响性能。应改为显式事务控制。
...
**建议**: 统一为单一缓存源，或让 TemplateEngine 直接使用 SiteConfigManager。
...
| **P0** | LLM/MCP Token 明文存储 | ⏭ 待处理 | 数据泄露 | 中 |
| **P1** | MCP 端点无速率限制 | ✅ 已修复 | 暴力破解 | 小 |
| **P1** | 裸 except 捕获 | ⏭ 待处理 | 错误隐藏 | 中 |
| **P1** | 搜索高亮代码重复 | ✅ 已修复 | 维护成本 | 小 |
| **P2** | 缓存一致性 | ⏭ 待处理 | 数据不同步 | 中 |
| **P2** | 缺少数据库索引 | ⏭ 待处理 | 性能瓶颈 | 小 |
...
1. **安全层面** — XSS 高亮注入、Cookie 安全属性缺失、OAuth CSRF、密钥明文存储
2. **代码健壮性** — 20+ 处裸 except 吞异常、缓存不一致、f-string SQL
